...
using travelcardlambdachsarp549.Services;
...
public class DbHelper
{
    ...
    public DbHelper()
    {
        ...
        var connectionString = $"Host={host};Port={port};Database={database};Username={username};Password={password};Pooling=true;Maximum Pool Size=20;Minimum Pool Size=0;Timeout=15;Command Timeout=30";
    ...
    public async Task<CreateTravelcardResponse> CreateTravelcardAsync(CreateTravelcardRequest request)
    {
        ...
        try
        {
            var travelcardToken = TokenGenerator.GenerateToken(6);
            const string sql = @"
...
RETURNING id;";
            ...
            var travelcardId = (int)(await cmd.ExecuteScalarAsync() ?? throw new InvalidOperationException("Insert failed"));
...
INSERT INTO public.cardholders (
    ...
    @cardholder_photo_key
                ...
                cardholderCmd.CommandText = cardholderSql;
            ...
            return new CreateTravelcardResponse(travelcardId, travelcardToken);
        }
        catch
        {
