...
public static class SecretHelper
{
    ...
    static SecretHelper()
    {
        ...
        if (!string.IsNullOrEmpty(uri))
        {
            ...
            {
                Console.WriteLine("[SecretHelper] Key Vault init failed.");
            }
        }
        else
        {
    ...
    public static string Get(string secretName, string envFallback)
    {
        ...
        if (_client != null)
        {
            try
            {
                ...
                Console.WriteLine("[SecretHelper] Secret resolved from Key Vault.");
                return value;
            }
            catch (Exception)
            {
                Console.WriteLine("[SecretHelper] KV secret not found — falling back to env var.");
            }
        }
