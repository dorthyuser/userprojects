...
public class DataSourceConfig
{
    ...
    {
        String keyVaultUri = System.getenv("AZURE_KEY_VAULT_URI");
        if (keyVaultUri == null || keyVaultUri.isBlank())
        {
            logger.warn("AZURE_KEY_VAULT_URI not set — Key Vault disabled, falling back to env vars");
            return null;
        }
        logger.info("Initialising SecretClient with Key Vault: {}", keyVaultUri);
    ...
    public DataSource dataSource(SecretClient secretClient)
    {
        try
        {
            ...
            logger.info("DataSource initialised — pool: syncspringboot109HikariPool");
