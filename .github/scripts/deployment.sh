...
FUNCTION_APP_NAME=$(echo "${PROJECT_NAME}-new")
...
# FIX 1: Storage names are globally unique — no --resource-group here
# or accounts in other groups cause StorageAccountAlreadyTaken error
...
if [[ -z "$APP_EXISTS" ]]; then
  ...
  # FIX 2: Set full runtime config on first create only
  # FIX 3: WEBSITE_RUN_FROM_PACKAGE=1 required for function keys to appear in portal
  ...
  # FIX 2: On redeploy — skip FUNCTIONS_WORKER_RUNTIME (causes Bad Request on Linux consumption)
  # FIX 3: Always keep WEBSITE_RUN_FROM_PACKAGE=1 so keys stay visible in portal
  az functionapp config appsettings set \
    --name "$FUNCTION_APP_NAME" \
    --resource-group "$RESOURCE_GROUP" \
    --settings \
