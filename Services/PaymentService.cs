...
public sealed class PaymentService : IPaymentService
{
    ...
    public async Task<PaymentDto> CreateAsync(PaymentUpsertRequest request, string correlationId, CancellationToken cancellationToken)
    {
        ...
        await using var connection = new NpgsqlConnection(_connectionString);
        await connection.OpenAsync(cancellationToken);
        await using var command = new NpgsqlCommand(@"insert into payments (user_id, order_id, amount, currency, payment_method, provider, provider_transaction_id, status, paid_at, description, metadata, created_at, updated_at) values (@user_id, @order_id, @amount, @currency, @payment_method, @provider, @provider_transaction_id, @status, @paid_at, @description, cast(@metadata as json), now(), now()) returning id, created_at, updated_at", connection);
        AddRequestParameters(command, request, icici.TransactionId);
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        await reader.ReadAsync(cancellationToken);
        return new PaymentDto((long)reader["id"], request.UserId, request.OrderId, request.Amount, request.Currency, request.PaymentMethod, request.Provider, icici.TransactionId, request.Status, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow, request.PaidAt, request.Description, request.Metadata);
    }
