...
public sealed class Service
{
    public async Task<CreateTravelcardResponse> CreateAsync(CreateTravelcardRequest request, ILambdaContext context)
    {
        ...
        var dataSourceBuilder = new NpgsqlDataSourceBuilder(BuildConnectionString());
        ...
        try
        {
            ...
            await InsertCardholdersAsync(connection, transaction, travelcardId, request.Cardholders, context);
...
RETURNING id;";
    ...
    private static async Task InsertCardholdersAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, int travelcardId, List<CardholderRequest> cardholders, ILambdaContext context)
    {
...
VALUES (@travelcard_id, @cardholder_title, @cardholder_forename, @cardholder_surname, @cardholder_type, @cardholder_photo_name, @cardholder_photo_rrs_key, @cardholder_photo_url, @cardholder_photo_key);";
    ...
    private static string GenerateToken(int travelcardId)
    {
        var bytes = System.Security.Cryptography.SHA256.HashData(System.Text.Encoding.UTF8.GetBytes($"{travelcardId}:{DateTimeOffset.UtcNow.UtcTicks}"));
        return Convert.ToBase64String(bytes).Replace("=", string.Empty).Replace("+", string.Empty).Replace("/", string.Empty)[..6];
    }
