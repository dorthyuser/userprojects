...
namespace zoho_project_csharp.Tests
{
    public class ZohoCrmConnectionTests
    {
        private class FakeClientFactory : IHttpClientFactory
        {
            ...
            {
                // Do not dispose handlers automatically to keep handlers usable across multiple clients in tests
        ...
        public async Task RefreshToken_SetsAccessToken_AndUsesApiDomain()
        {
            ...
            // Authorization header may be available via Authorization property or raw header values
        ...
        public async Task SendAsync_Unauthorized_RetriesAfterRefresh()
        {
            ...
            Assert.Equal(HttpStatusCode.OK, resp.StatusCode);
            Assert.Equal(2, tokenCalls); // initial refresh + refresh after 401
        ...
        public async Task RefreshToken_InvalidJson_ThrowsInvalidOperationException()
        {
            ...
            var apiHandler = new DelegatingHandlerStub((req, ct) =>
            {
                // This should not be reached because token parsing fails
        ...
        public async Task ShortExpiry_TokenRefreshedOnEachCall()
        {
            ...
            Assert.Equal(2, tokenCalls); // refreshed twice due to short expiry
        }

        // simple delegating handler stub to allow synchronous-like control in tests
