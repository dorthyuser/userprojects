...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def request_validation_exception_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
        logger.error(json.dumps({"message": "request_validation_error", "path": str(request.url.path), "errors": _sanitize_errors(exc.errors())}, default=str))
    ...
    async def generic_exception_handler(request: Request, exc: Exception) -> JSONResponse:
        logger.error(json.dumps({"message": "unhandled_exception", "path": str(request.url.path), "error": str(exc)}, default=str), exc_info=True)
