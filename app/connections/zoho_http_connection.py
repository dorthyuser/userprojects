...
class ZohoHttpConnectionConnection:
    ...
    def request(self, method: str, path: str, params: dict[str, Any] | None = None, json: Any | None = None, headers: dict[str, str] | None = None) -> tuple[int, Any]:
        self._ensure_token()
        request_headers: dict[str, str] = {"Authorization": f"Zoho-oauthtoken {self._access_token}"}
        if headers:
            request_headers.update(headers)
        url = urljoin(self._api_domain.rstrip("/") + "/", path.lstrip("/"))
