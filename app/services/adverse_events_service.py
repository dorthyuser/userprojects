...
def _validate_query_params(
    ...
    if ctcaeGrade is not None and ctcaeGrade not in {1, 2, 3, 4, 5}:
        ...
        raise HTTPException(status_code=502, detail="INVALID_QUERY_PARAM")
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if cursor.fetchone() is None:
                raise HTTPException(status_code=502, detail="TRIAL_NOT_FOUND")
            cursor.execute(
                "SELECT id FROM trial_enrolments WHERE trial_id = %s AND patient_id = %s AND status = 'ENROLLED'",
                (payload.trialId, payload.patientId),
            )
            if cursor.fetchone() is None:
                raise HTTPException(status_code=502, detail="PATIENT_NOT_FOUND")
            cursor.execute(
            ...
            if duplicate is not None:
                raise HTTPException(status_code=409, detail={"code": "DUPLICATE_AE", "aeId": duplicate[0]})
        ...
        try:
            ...
            sns_published = False
            sns_message_id = None
        return AdverseEventCreateResponse(status="success", aeId=ae_id, notificationId=notification_id, snsPublished=sns_published, snsMessageId=sns_message_id, receivedAt=_utc_now())
...
def get_notifications(
    ...
    try:
        ...
        notifications = [NotificationItem(notificationId=row[0], aeId=row[1], trialId=row[2], siteId=row[3], patientId=row[4], aeTermName=row[5], ctcaeGrade=row[6], serious=row[7], priority=row[9], outcome=row[8], acknowledged=row[10], snsPublished=row[11], createdAt=row[12]) for row in rows]
