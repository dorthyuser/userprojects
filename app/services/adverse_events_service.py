...
def submit_adverse_event_service(payload: AdverseEventCreateRequest) -> AdverseEventCreateResponse:
    ...
    try:
        ...
        conn.rollback()
        logger.error(json.dumps({"event": "database_error", "message": str(exc)}), exc_info=True)
...
def get_notifications_service(trialId: str | None, siteId: str | None, ctcaeGrade: int | None, serious: bool | None, acknowledged: bool | None, priority: str | None, dateFrom: str | None, dateTo: str | None, page: int, pageSize: int) -> NotificationListResponse:
    ...
    if page < 1 or pageSize < 1 or pageSize > 100:
        _raise_validation("INVALID_PAGINATION")
