...
from app.services.prime_service import get_prime_service

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/prime", tags=["prime"])
service = get_prime_service()


@router.get("", response_model=PrimeResponse, status_code=status.HTTP_200_OK)
async def get_prime(request: Request) -> PrimeResponse:
    logger.info(json.dumps({"event": "route_entry", "method": request.method, "path": request.url.path, "resource": "prime"}))
    try:
        return service.get_prime_between_100_and_200()
